...
public class LoggingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            // Reset stream and copy back to original
            context.Response.Body.Seek(0, SeekOrigin.Begin);
            await responseBody.CopyToAsync(originalBodyStream);
        }
        catch (Exception ex)
        {
            // Always restore body stream before throwing
            context.Response.Body = originalBodyStream;

            await HandleExceptionAsync(context, ex);
            throw; // Let UseExceptionHandler handle it
        }
        finally
        {
            // Make sure we restore the original stream in all cases
            context.Response.Body = originalBodyStream;
        }
    }
    ...
    {
        _logger.LogError(ex, "An unhandled exception occurred while processing request. Path: {Path}", context.Request.Path);
